...
import {
  getGroundOwnerBookings,
  updateBookingStatus,
  cancelBooking,
  completeBooking,
} from '@/services/bookingsService';
import { BookingListItem, statusToLabel } from '@/types/bookings';

export type BookingStatus = 'Confirmed' | 'Pending' | 'Cancelled' | 'Completed' | 'Rejected';
...
export interface Booking {
  ...
  facility: string;
  pitch?: string;
  sport: 'Cricket' | 'Football' | 'Tennis' | 'Badminton' | string;
  date: string;
  startTime: string;
  endTime: string;
  durationHrs: number;
...
function mapApiBooking(b: BookingListItem, index: number): Booking {
  ...
  const startMinutes =
    parseInt(startParts[0] ?? '0', 10) * 60 + parseInt(startParts[1] ?? '0', 10);
  const endMinutes =
    parseInt(endParts[0] ?? '0', 10) * 60 + parseInt(endParts[1] ?? '0', 10);
  const durationHrs = Math.round(((endMinutes - startMinutes) / 60) * 2) / 2;

  const statusLabel = statusToLabel(b.status);
  const displayName = b.userEmail ?? 'Unknown';
  const initials = displayName.substring(0, 2).toUpperCase();
  const avatarColor = AVATAR_COLORS[index % AVATAR_COLORS.length] ?? 'bg-gray-100 text-gray-700';
  ...
  return {
    ...
    facility: b.groundName ?? 'Unknown Ground',
    sport: 'Cricket',
    date: dateStr,
    startTime: formatTime(b.startTime),
    endTime: formatTime(b.endTime),
    durationHrs,
    totalPKR: b.totalAmount,
    advancePKR: b.advanceAmount,
...
const TABS = ['All Bookings', 'Pending', 'Confirmed', 'Completed', 'Cancelled', 'Rejected'];
...
export const BookingsList: React.FC = () => {
  ...
  const [selectedGround, setSelectedGround] = useState('All Facilities');
  ...
  useEffect(() => {
    getGroundOwnerBookings()
      .then((res) => setBookingsList(res.bookings.map((b, i) => mapApiBooking(b, i))))
      .catch((err) =>
  ...
  const groundNames = [
    'All Facilities',
    ...Array.from(new Set(bookingsList.map((b) => b.facility))),
  ];

  const handleUpdateStatus = async (bookingId: string, newStatus: BookingStatus) => {
    const booking = bookingsList.find((b) => b.id === bookingId);
    const realId = booking?._realId ?? bookingId;

    try {
      if (newStatus === 'Confirmed') {
        await updateBookingStatus(realId, 2);
      } else if (newStatus === 'Cancelled') {
        await cancelBooking(realId);
      } else if (newStatus === 'Completed') {
        await completeBooking(realId);
      }
    } catch {
  ...
  const filteredByGround =
    selectedGround === 'All Facilities'
      ? bookingsList
      : bookingsList.filter((b) => b.facility === selectedGround);
    ...
    <div className="w-full space-y-5 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Manage and track all facility{' '}
            <span className="text-emerald-600 font-medium">reservations</span>.
          </p>
        </div>
      ...
      <div className="inline-flex items-center gap-3 bg-white border border-gray-200 rounded-xl px-4 py-2.5 shadow-2xs">
        <div>
          ...
          <p className="text-base font-black text-gray-900">PKR {revenue.toLocaleString()}</p>
        </div>
                ...
                filtered.map((b, idx) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ gridTemplateColumns: COL }}
                    >
                      <span className="text-xs font-bold text-gray-400">#{b.id.split('-')[0]}</span>
                      ...
                      <div className="flex items-center gap-2.5 min-w-0 pr-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-gray-900 truncate">{b.player.name}</p>
                          <p className="text-xs text-gray-400 font-medium truncate">{b.player.role}</p>
                        </div>
                      </div>
                      ...
                      <div className="min-w-0 pr-3">
                        ...
                        <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                          <span>{SPORT_ICONS[b.sport] ?? '🏟️'}</span>
                          <span>{b.sport}</span>
                        </p>
                      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between bg-gray-50/80 p-4 rounded-2xl border border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-gray-900 text-sm">{selectedBooking.player.name}</h3>
                  <p className="text-xs text-gray-500 font-medium">
                    {selectedBooking.player.phone || 'No phone on file'}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="bg-white border border-gray-200/80 shadow-2xs rounded-2xl p-4 space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[11px] text-gray-500 font-medium">Facility &amp; Pitch</p>
                    <p className="text-xs font-bold text-gray-900 mt-0.5">{selectedBooking.facility}</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-gray-900 mt-0.5">{selectedBooking.date}</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-extrabold text-gray-900 mt-0.5">#{selectedBooking.id}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="bg-white border border-gray-200/80 shadow-2xs rounded-2xl p-4 space-y-3 text-xs">
                <div className="flex justify-between items-center text-gray-600">
                  <span>Total Fee ({selectedBooking.durationHrs} hrs)</span>
                ...
                <div className="flex justify-between items-center text-gray-600">
                  ...
                  >
                    PKR {selectedBooking.advancePKR.toLocaleString()} (
                    {selectedBooking.advancePaid ? 'Paid' : 'Unpaid'})
                  </span>
                </div>
